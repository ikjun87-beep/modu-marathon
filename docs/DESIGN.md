...
- **공유 카드**(최고 ROI) — ✅ 2026-07-30 구현: 러닝 상세 → [공유] → 미리보기 시트에서 **정사각(1080)/스토리(1080×1920)** 선택, 크루 사진 배경(선택), 시스템 공유 시트로 인스타·카톡 전송. `components/share-card.tsx`(SVG) + `share-sheet.tsx` + `lib/{share-layout,text-metrics,share-image,path-fit}.ts`.
  ...
  - **거리 링은 10km에서 포화된다 → 그 위는 마일스톤이 대신 말한다**(2026-08-02). 링은 `RING_FULL_KM=10`에 `Math.min(…,1)` 클램프라 10km·하프·풀코스가 **완전히 같은 그림**(꽉 찬 원 + 12시 골드점)이었고 링 안쪽은 비어 있었다 — 인스타에서 카드를 본 사람은 밑의 숫자를 읽기 전에 그림부터 보는데, 그 그림이 42km와 10km를 구분하지 못했다. 처방 3가지: ①**꽉 차면 링이 골드**로 바뀐다(9km 그린 90% ↔ 12km 골드 100%가 한눈에 갈린다. 10km 완주는 성과이므로 "골드=순위·챌린지·성과 전용" 규칙에 부합) ②**비어 있던 링 안쪽에 마일스톤 라벨**(10K / 하프 / 풀코스, `share-layout.ts`의 `milestoneFor()`). **10km 미만엔 두지 않는다** — 채우는 중인 초록 링 안의 "5K"는 카드 아래 `6.42 km`와 어긋나 보여 **라벨이 이 러닝의 거리로 오독된다.** 꽉 찬 골드 원 안에 들어갔을 때만 메달의 각인처럼 읽힌다(5km 자랑은 배지 카드가 맡는다) ③꽉 찬 링에서는 **12시 출발점을 찍지 않는다**(골드 위 골드라 안 보이고, 같은 그림을 만들던 요소였다).
    - ⚠️ 마일스톤 라벨은 **골드가 아니라 본문색(`ink`)**이다. 크림 배경 위 골드는 **2.86:1**로 큰 텍스트 기준(3:1)에도 미달한다 — 세션8(다크 위 블루 3.06:1)·세션9(딥올리브 위 골드 4.30:1)과 같은 함정이다. 성과 신호는 링이 골드로 내고, 글자는 읽히는 게 먼저다.
    - ⚠️ 기준 거리는 **공식 거리 정각(42.195/21.0975)이 아니라 42.0/21.0**. 수동 입력은 "42.19"로 들어오고 GPS도 접선 오차로 흔들려, 정각을 요구하면 **실제 완주자가 라벨을 못 받는다**. 반대로 41km를 풀코스라 부르면 거짓이므로 공식 거리의 −0.5%에서 끊는다. 10은 앱 배지(10K 러너)와 같은 값이다.
    - 같은 어휘를 **앱 목록 썸네일**(`components/distance-thumb.tsx`)도 쓴다 — 꽉 찬 링은 거기서도 골드. 앱과 밖으로 나가는 이미지가 같은 말을 해야 한다.
    - ⚠️ **걷기는 카드·썸네일 양쪽에서 제외**한다. 이 앱에서 걷기는 랭킹·배지에서 빠지는 기록이라, 12km를 걸었다고 골드·"10K"를 주면 성과 신호가 뜻을 잃는다(그리고 걸어서 "풀코스"는 거짓말이 된다).
- **지도는 장식이 아니라 정보**(2026-07-31 회장 지시 — 통근버스 앱 대비 "저기는 또렷한데"):
  커스텀 파스텔 스타일이 배경(크림)과 도로(흰색)를 같은 밝기로 만들고 **도로·지명 라벨을 전부 꺼서**, 내가 어디를 달리는지 알 수 없는 지도가 돼 있었다. → **거의 기본 구글 지도**로 되돌리고 잡음만 걷어낸다(상점 로고·지적선·교통 아이콘). **지명·도로·하천·공원·역 이름은 살린다.** 브랜드 톤은 지도 위에 얹히는 UI(버튼·카드·경로선)가 담당한다 — Strava·NRC가 표준 지도를 쓰는 이유와 같다.
  - 다시 브랜드 톤 지도로 갈 땐 두 가지를 지킬 것: ①배경과 도로의 **명도 차이 확보** ②**라벨을 끄지 말 것**(끄는 순간 지도가 아니라 무늬가 된다). 이전 스타일은 `MAP_STYLE_PASTEL`로 보관.
  - **경로선은 두 겹** — 흰 테두리(11) 위에 브랜드 그린(6.5). 기본 지도는 도로가 흰색·노랑, 공원이 초록이라 한 겹 그린은 배경에 따라 묻힌다(티맵·카카오네비가 쓰는 기법).
  - 라이브 러닝엔 **내 위치 버튼 + 줌 버튼**을 둔다. 달리면서 핀치 줌은 어렵고, 지도를 밀어 옮겼을 때 돌아올 방법이 필요하다.
  - **모달을 열면 시작 전에도 현재 위치로 지도를 옮긴다.** 안 그러면 서울 기본 좌표에 머물러 "어디서 출발하는지"가 안 보인다. 단 **여기서 권한을 요청하지 않는다** — 이미 허용한 사람에게만 미리보기를 주고, 안 물어본 사람에겐 [러닝 시작]에서 묻는다(화면 열자마자 권한 창은 놀란다).
