...
> ## ✅ 2026-07-30 확정 — 회장 결정 + 실행 완료
> **회장 결정: "당신 안대로 갑니다"** — 팔레트 **안 A(크림 러너)**, 마스코트 **안 1(양)** 채택.
> 서지안(plan-ux)의 반대 검토(`docs/MASCOT_UX_REVIEW.md` — 세션8이 이미 색 없이 79→94를 증명했으니
> ROI가 낮다, 3050대 남성 확산에 부끄러움 리스크, 우선순위는 공유카드·스토어 출시가 먼저라는 주장)를
> 회장이 청취하고도 **명시적으로 기각**했다 — 이 결정은 종결된 것으로 취급하고 재론하지 않는다.
> 단, §3(확산 리스크)은 **완전히 사라진 리스크가 아니라 회장이 감수하기로 한 리스크**이므로,
> 실기기 배포 후 실제 크루원 반응이 예상과 다르면 그 실측 데이터로 재논의할 여지는 남겨둔다.
>
> **실행 완료물**: `app/assets/images/mascot-{m,f}-{red,green}.png`(양 4종, 신규 파이프라인
> `app/scripts/{cutout-mascot-bg,recolor-mascot-vest}.mjs`) · `app/src/lib/brand.ts`(팔레트 안 A,
> 회장 파일럿 + 황태경 마무리) · `docs/DESIGN.md` 갱신. 세부는 아래 자문 본문의 §1 안A·§2 안1 그대로.
>
> **회장 파일럿의 다크 보정(#26301f→#171c12) — 동의.** 골드 대비가 4.30:1(AA 미달, 시상대 배지
> 숫자 크기 기준으로도 아슬아슬)에서 5.41:1(현행 네이비와 동급)로 회복된다. 실측 재확인 완료.
>
> **`Team.green` 충돌은 여전히 미해결** — 이번 스코프에서 억지로 정하지 않았다. 마스코트 조끼의
> 그린은 기존 `Team.green`(#2f8f5b)을 그대로 썼다(팀 정체성 일관성 우선) — 즉 "브랜드 그린 vs
> 팀 그린이 헷갈린다"는 문제는 **마스코트가 아니라 아바타 링 쪽에서만** 남아 있다. 다음 세션에서
> 팀 이름(레드/그린) 자체를 바꿀지, 링 색만 다른 계열로 옮길지 결정 필요.
>
> **추가로 잡은 것(자문 시점엔 없던 실전 버그)**: 실제 코드 정밀 스윕에서 회장 파일럿이 놓친
> 하드코딩 블루 잔재 4곳을 더 찾아 고쳤다 — `animated-icon.web.tsx`가 참조하는 CSS 모듈(네이티브만
> 고치고 웹 변형을 놓침), `Shadow.soft/card`의 그림자색(Brand 객체 밖에 있어 자동 반영 안 됨),
> `schedule-section.tsx`의 하드코딩 네이비 칩 텍스트(`Brand.brandDeep` 토큰으로 교체 — 8.28:1),
> `badge-celebration.tsx` 컨페티의 하늘색 파편(`Team.red`로 교체). 이 패턴 — **`Brand` 객체 밖에
> 있는 색 토큰(Shadow·Hairline)과 컴포넌트별 하드코딩 hex는 브랜드 교체 시 자동으로 안 따라온다**
> — 은 다음에 팔레트를 또 바꿀 때도 반복될 함정이니 전역 규칙 후보로 남긴다.
