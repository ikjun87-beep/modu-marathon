...
- [ ] **13차 실기기 확인**(회장): ①앱 정상 실행 ②5탭 블루 ③마이 탭 → 계정 → 로그인/게스트 ④**앱 껐다 켜도 로그인 유지**(RN 영속성 핵심). → 만족 시 `feat/p7-2-shared-motion`→main 머지. **머지 전 체크: `demo.ts` DEMO=false**(검토용 데모 끄기 — 실배포 상태로).
