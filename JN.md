...
> 최종 업데이트: 2026-07-29 **세션8** (**🎨 디자인 전면 개편 R12~R15 — 독립 채점 79→87→89점 + 🏷 앱이름 『5키로』 + 👏박수·커스텀 프로필** — 브랜치 `feat/design-polish-r1-r9` HEAD=`1e2b1eb`, **13커밋 미푸시**. ⚠️ **배포 일절 안 함**(회장 지시: 완성 후에). ①**서브에이전트 4회 투입**(회장 승인): 황태경(build-design)이 실기기 캡처로 **독립 채점**, 서지안(plan-ux)이 Strava·NRC·런데이·삼성헬스·토스 **티어다운**. ②**충격적 진단 — 제 자체 판단 95점은 거짓, 실제 79점**. 감점이 규칙(90%대)이 아니라 **구조**(레이아웃 11/15·감성차별성 8/15)에 몰려 있어 부분 수정으론 못 올린다는 게 밝혀짐. "AI스럽다"의 확정 원인 3가지 = ①단일 카드 템플릿 복사 ②시그니처(다크카드)가 홈·러닝에 반복돼 **두 번째 템플릿**이 됨 ③블루+화이트+그레이뿐인 팔레트(=v0/Bolt류 AI 스캐폴딩 문법). ③**경쟁 분석의 뼈아픈 발견**: 처방이 이미 `DESIGN.md` "앱 디자인 큐"에 **공유카드·👏박수·지도썸네일·다크모드**로 적혀 있는데 **전부 미구현**이었다 — 새 발명이 아니라 밀린 숙제. 이후 그 절에 ✅/⬜ 구현현황 표기 도입. ④**처방 = 탭마다 다른 레이아웃 언어**: 홈=**크루 타임라인**(아바타가 레일 노드) / 크루=**사진 우선 2열**+👏박수 / 러닝=**경로 SVG 폴리라인 썸네일**+가로 스트립 / 랭킹=**가로 시상대**(2위-1위-3위, 단 높이=순위) / 마이=**프로필 히어로**. **다크 카드는 랭킹 시상대 전용**으로 회수. ⑤**색은 안 늘리고** ①`Brand.tint` 표면 단계 ②**아바타 링 색을 이름 해시로**(브랜드·레드·그린 3색) → 여러 명 나오는 화면이 자동으로 다채로워짐(실데이터로 증명). ⑥**신규**: `Avatar`·`RouteThumb`·`DistanceThumb`(경로 없으면 거리를 링 채움으로, 10km=한바퀴)·`ClapButton`(`claps` 컬렉션)·`lib/feed.ts`(타임라인 병합)·`lib/profile-photo.ts`(커스텀 프로필, **기기 로컬** — 공개 읽기 컬렉션에 얼굴 사진은 처리방침 개정 선행 필요)·`clock` 아이콘. ⑦**『5키로』 리브랜딩**: app.json name만 교체(**slug·scheme·package는 유지** — 바꾸면 새 앱이 돼 데이터 끊김), 온보딩 "오늘 5키로, 오키?"(마스코트 엄지척과 페어링), HC 안내 문구, 개발자 용어 추방("Firebase 미설정"→"아직 크루와 연결되지 않았어요"). ⑧**마스코트 방향 = 팀 위임 결정, 재생성 불필요**: 라벨 "남/여"→**"숏컷/포니테일"**(성별 신고로 포장할 이유 없음), **Avatar 실제 결함 수정**(me면 크기 무관 512px 원본을 22~30px로 줄여 뭉갬 → 36px 미만은 이니셜), **성장 링**(배지 수 → 회색/팀색/블루/골드, 마이 히어로에만). ⑨**채점이 잡아낸 내 버그 2건**: (a) 러닝 탭 `OOO님의 기록` 라벨이 **크루 전체 목록**을 가리킴 — `listData`엔 원래 이름 필터가 없고(설계상 크루 전체), 그 라벨은 [직접 입력] 폼을 가리키던 건데 **R10에서 폼을 목록 아래로 옮기며 라벨만 남아 거짓말**이 됐다. 데이터가 내 것뿐일 땐 우연히 맞아 안 보였다 → `크루의 지난 러닝`+`OOO님으로 저장돼요`. (b) 홈 다크카드를 걷어내며 **"이번 주 우리 크루" 지표가 통째로 실종** → 띠로 복원. ⑩**다크 위 블루 AA 미달 실측**: `#2563c9` on `#141a2b` = **3.06:1**(AA 4.5:1 미달, 단위는 11~13px라 "큰 텍스트 3:1" 완화도 못 받음) → **`Brand.brandOnDark` #4a8ae8 = 5.02:1** 신설. ⑪**실데이터 시딩이 결정적**: 크루원 4명·러닝 12건·걷기 2건을 넣자 데이터 0일 땐 안 보이던 결함이 드러남 — **"랭킹 탭인데 순위가 스크롤 밖"**(챌린지·리포트가 첫 뷰포트 점유) → 시상대를 맨 위로. 시상대 단이 라운드 모서리 뚫고 잘림 → `overflow:hidden`. **캡처 후 16건 전량 삭제 완료**. ⑫**빌드 락 함정**: `flock`은 파일이 아니라 fd에 잠금 → **락 파일은 안 지워진다**. `[ -f $LOCKFILE ]`로 판단해 **14시간을 헛돌았다**. `pgrep -f "gradlew assembleRelease"`도 자기 자신이 잡힌다. 올바른 대기법을 `build-local-apk.sh` 주석에 박음. ⑬**채점 6회 완주 — 79→87→89→88→93→94점**(목표 95, **1점 미달**). R15에서 **점수가 내려간(-1)** 게 전환점이었다: 채점자가 *"고쳤다는 커밋과 검증했다는 캡처를 열어보니 절반이 실제로 그 문제를 해결한 게 아니었다"* — 확인해보니 `V-06-walk.png`가 `V-07`과 **바이트까지 동일**했고(같은 화면을 두 번 찍고 이름만 다르게 붙임), 랭킹 캡처가 수정 커밋보다 **9분 먼저** 찍혀 있었다. → **캡처 무결성 검사**(md5 중복 + 커밋 시각 이후) 도입(`scratchpad/verify-shots.sh`), 실제로 중복을 한 번 더 잡아냈다. ⑭**반복된 내 실수 패턴 = "한 곳만 고치고 같은 패턴의 다른 곳을 놓친다"**: 다크 위 블루(시상대만 고치고 상세 빼먹음)·걷기 구분(히어로만 고치고 상단 제목 빼먹음)·걷기 태그(상세에서 빼고 목록은 놔둠). → 이후 **고치기 전에 전수 검색**하는 방식으로 전환(`Brand.dark` 배경 5곳·`numberOfLines={1}` 이름 8곳 전수 확인). ⑮**긴 이름(19자, 한계 20자) 처방 = 화면별로 다르다**: 목록 행은 **3줄 허용**(풀네임 책임), 시상대는 **한 줄 고정 + 글자 축소 85%**(정렬이 생명). 시상대에 2줄을 허용했더니 1위 칸만 길어져 **2·3위와 정렬이 무너졌고**, 축소를 75%까지 뒀더니 **1등 이름이 2·3위보다 작아져 위계가 뒤집혔다** — 둘 다 실기기가 잡았다. **같은 문제라도 폭 예산이 다르면 처방이 달라야 한다**는 게 이번 최대 교훈. **다음 첫액션**: ①R18 채점(남은 1점) ②**완성 후 배포 3건 한 번에**(`claps` 규칙 배포 / 미푸시 커밋 푸시 / 프로필 사진 공유저장 여부 결정=처리방침 개정 선행). **미검증**: 온보딩 첫화면(클린 설치 필요)·GPS 라이브 모달·계정 시트·워치 연동(권한은 회장 손으로 — 채점자도 이 제약은 감점에서 제외).)<br>이전 세션7: 2026-07-27 **세션7** (**📱 25차 APK 실기기 검증 + 🎨 디자인 감사 R10 = 87점→95점 목표 도달** — 브랜치 `feat/design-polish-r1-r9` HEAD=`a7c37ac`. ①**R8·R9 미검증분 전부 정상 확인**: 러닝목록 행 넘침 해소(320dp에서도 `6.40km 48:16 7'33"/km ♥143` 한 줄)·Black Han Sans 숫자·랭킹 1위 시그니처·마스코트 4종 포니테일 구분. 세션6 후반 커밋 4건(러닝 히어로=내 기록·워치 자동로드·다이얼로그 블루·빌드 부하)도 확인. ②⚠️ **검증 기준기 발견 — 테스트폰이 320×711dp**(디스플레이 배율로 density 480→540). 우리 기준 360×800보다 12.5% 좁다. "글자가 커서 한 화면을 넘어간다"의 또 다른 원인이 여기 있었다. CLAUDE.md에 명문화. ③**R10 감사 = 실기기 전 화면 캡처 대조로 28건 수정**. 규칙은 문서에 있는데 화면마다 적용이 갈려 있던 것들: **표기(숫자=본문색+단위=블루)** 6곳(월간리포트 6칸 회색·마이 `0회`·상세 `/km`·목록 심박이 통짜 블루라 숫자가 파랗게 튐·목록 `/km`·GPS 트래킹) → **단위엔 예외 없음**(회·/km·bpm·m·일 전부)으로 규칙 확정. **색**(골드=순위·챌린지·성과 전용) 3곳(랭킹 챌린지 아이콘 블루솔리드→골드·홈 방명록/검색 아이콘 골드→블루·GPS [계속] 골드→블루). **위계** — 크루에 솔리드 3개였던 것을 주 액션 [참석] 하나로, 세그먼트를 **트랙+흰 pill**로 통일(러닝 탭만 낱개 칩이라 계정 시트와 갈렸음), 카드는 그림자로(댓글·GPS 스탯·온보딩이 테두리만 있어 납작), 홈 랭킹·마이 바로가기는 탭바 중복이라 톤온톤 칩으로 낮춤. **버그 5건**: 랭킹이 **1위인데 "한 번 더 뛰면 순위가 올라가요"**(→순위별로 갈라 "2위와 1.4km 차이" 실제 격차)·1위 시그니처 카드 이름 `TESTM2…` 잘림(→아바타·거리 축소 + "(나)"를 라벨줄 "내가 이번 주 1등!"으로)·랭킹 빈 상태 안내문이 탭바 뒤로 밀려 마스코트만 보이는 막다른 화면(→가로 배치로 높이 절반)·계정 시트 비활성 [로그인]이 연블루+흰글자로 **대비 1.5:1**(AA 미달)·첫 달엔 리포트 6칸이 전부 "새 기록" 초록 도배. **정보 위계**: 러닝 탭 '직접 입력'이 첫 뷰포트를 먹고 '지난 러닝'을 밀어냈다(→FlatList footer로)·마이 탭만 페이지 타이틀 없음(→MY/내 프로필)·마이 캐릭터 4종이 화면 절반을 먹어 '내 러닝'이 밖으로(→접고 [바꾸기])·상세 '기록 방식' 타일이 메타줄과 중복(→제거)·상세 '시간'에 심박 파형 아이콘(→`clock` 아이콘 신규). ④**빌드 4회·설치 4회로 라운드마다 실기기 재검증**, tsc 0에러·expo export web 그린. ⑤**USB(usbip) 패스스루가 전송 중 계속 끊겨**(50MB 설치 2회 실패) **Wi-Fi adb로 전환** — `adb tcpip 5555` → `adb connect 192.168.11.226:5555`. 끊기면 usbipd 재부착→tcpip 재활성화하는 `ensure()` 헬퍼로 무인 진행. **⚠️ 미검증 2건**: GPS 라이브 모달(열면 위치 권한 창이 떠서 회장 대신 허용/거부하지 않음)·온보딩 첫화면(클린설치 필요). **다음 첫액션**: ①GPS 모달·온보딩을 회장이 직접 1회 확인 ②앱이름 `5키로` 적용 + 커스텀 프로필 사진 등록(갤러리 base64 방식 재사용) ③홈 하단 여백(데이터 1건씩일 때 두드러짐 — 배지 진행 카드 후보).)<br>이전 세션6: 2026-07-27 **세션6** (**🎨 디자인 감사 R1~R9 = 64점→87점 + 🏃 마스코트 4종 재생성 + 📱 타이포 중간크기 + 🔤 폰트교체 불필요 결론 + 🏷 앱이름 `5키로` 방향** — 브랜치 `feat/design-polish-r1-r9` HEAD=`83aa9a4`. ①**24차 APK 실기기 검증 완료**(새 테스트폰 갤럭시S21·Android15, 무선디버깅 재페어링): 월간리포트·모임카드칩·러너네임[저장]버튼 3건 정상. ②**타이포 중간크기 조정**(회장 지적 "글자 커서 아기자기함 사라지고 한화면 넘어감"): 페이지타이틀 34→26 통일·홈히어로 54→46·러닝상세 58→50 등 큰글자 ~12~15% 축소, 본문(12~16pt)은 유지. 원인=LINE Seed가 같은 pt에서 시스템폰트보다 크게 렌더(크기를 키운 게 아니었음). ③**모임 참석체크 카드 정리**: 페이스 조 제거(위치·시간·목표만)·참석배지 두 줄 넘침 수정. ④**지난 러닝 = 최근 1주 + 걷기/달리기 세그먼트**(기본 달리기, 전체도 선택 가능). ⑤**디자인 감사 루프**(build-design 20년차, 라운드마다 실기기 캡처 채점): R1 64 → R2 77 → R3·R4 79 → R5 81 → R6 85 → **R7 87**. 반영: 카드 elevation(Shadow 토큰)·버튼 위계(주=솔리드/보조=톤온톤)·랭킹 리더보드 리치화(금은동 순위뱃지+아바타+상대거리바)+**1위 시그니처 레이아웃**·이모지 메달 제거·탭바 검은띠 해소(userInterfaceStyle=light 고정)·**거리 소수점 규칙**(집계1자리/개별기록2자리, run.ts 명문화)·**숫자표기 전역규칙**(숫자=본문색+단위=브랜드블루)·**골드는 순위·챌린지 전용**으로 회수(페이스·참석·본문숫자에서 제거)·인풋 Radius.input(14) 5곳 통일·placeholder 전용 토큰·크루 칩 대비 AA·전화면 하단여백 160(탭바 클리핑)·러닝목록 행 넘침(6.4km~심박 칸 초과) 수정. ⑥**마스코트 4종 전면 재생성**(gpt-image-1): 남/여 차이가 볼터치 색뿐이라 썸네일에서 4종이 2종처럼 보였음 → 여성에 **긴 포니테일**로 실루엣 구분 + 마이탭 라벨(남·여 × 레드·그린). AI가 프롬프트 무시하고 넣는 발밑 그림자는 `app/scripts/strip-mascot-shadow.mjs`(연결덩어리 분석) 신규 작성해 제거, 알파채널로 4종 검증. ⑦**폰트 자문 결론 = 교체 불필요**: 회장 "디자인이 AI스럽다" 지적 → 원인은 폰트가 아니라 **컴포넌트 반복의 단조로움**(카드 반경+그림자+아이콘뱃지+제목/값이 전 화면 동일). Pretendard는 국내 AI 스캐폴딩 기본값이라 역효과, 스포카는 한글 2350자만 커버(러너네임 깨짐), 나눔스퀘어는 톤 불일치 → **LINE Seed 유지**. 처방=①숫자 전용 서체 **Black Han Sans** 이식(웹과 시각DNA 일치, 큰 숫자만) ②탭당 1곳 시그니처 모먼트(랭킹 1위 적용) ③마스코트 상태 반응(후행). ⑧**스크롤 정책 결정**: "스크롤=스트레스"는 폐기된 통념(NN/g) → 한 화면에 욱여넣지 않고 **첫 뷰포트=상태요약1+CTA1+최근항목1**로 분리, 기준 360×800dp(실사용 550dp), 큰 화면은 다음 카드가 더 보이는 식으로 흡수. 탭별 first-viewport 지정표 자문서 확보. ⑨**앱 이름 방향 = `5키로`**(회장 결정, 당근·삼쩜삼식 친숙어 패턴). 슬로건 후보: "오늘 5키로, 오키?"·"5키로면 충분해"·"가볍게, 같이, 5키로". **표기(5키로/오키로)·적용시점은 95점 완주 후**. ⚠️ **R8·R9 빌드분 실기기 미검증**(회장 이동으로 중단): 러닝목록 넘침 해소·Black Han Sans 숫자·랭킹 1위 시그니처·새 마스코트 4종. **다음 첫액션**: ①`cd app && bash scripts/build-local-apk.sh` 재실행(캐시로 빠름) ②실기기 설치·전화면 캡처 ③build-design 채점 → **95점 목표** ④완주 후 = 앱이름 `5키로` 적용 + **커스텀 프로필 사진 등록**(갤러리 base64 업로드 방식 재사용). 미푸시였던 세션5 커밋들도 이번에 함께 푸시.)<br>이전 세션5: 2026-07-19 **세션5** (**📅 일정이관 실기기검증 + 💾 이름 [저장]버튼 + 🎴 모임카드 칩 + 📊 월간리포트(골프벤치마크) + 🚩 배포방향=플레이스토어** — main HEAD=`1d32d54`(미푸시). ①**일정 Firestore 이관 실기기 검증완료**: 클린설치인데 events 3개 자동시딩·예전 참석데이터 id그대로 붙음·7/5 지난모임 자동접힘·모임만들기 시트 정상(연도표시). ②**크루탭 러너네임 [저장]버튼**(회장 지적: 자동저장이라 실수로 오염 → 마이탭처럼 draft+버튼, onBlur자동저장 제거). 23차 실기기 확인. ③제 adb 실수로 생긴 유령참석자·방명록(쭌쭌샷) 삭제. ④**배포방향 정정**: Firebase=데이터DB(용량충분·무관) vs APK=설치파일. GitHub/APK 사이드로드 접음 → **플레이스토어 정식출시**($25·AAB·심사, 급하지않음). 현재지시=배포 나중·앱 기능집중. ⑤**골프앱 벤치마크**(GDR·스마트스코어) → **월간리포트**(stats.monthReport: 이번달vs지난달 6지표+주별페이스추이, 랭킹탭) + **모임카드 칩**(parseEventInfo로 장소·시간·거리 분리, 📍⌚🏃). **⚠️ 24차 APK 빌드완료·실기기 미검증**(무선디버깅 꺼짐). **다음: 24차 검증(리포트·모임카드·저장버튼) → 회장 레퍼런스 대기(홈지표3칸·설정화면톤·러닝상세 리치화 후보).**<br>이전 세션4: (**🎨 상승고도·아기자기함·폰트·마스코트4종 + 🛡️ 적대검토4렌즈 + 🗺 로드맵재정렬 + 📅 일정 Firestore 이관·모임만들기** — **main 머지 완료(HEAD `33fa817`+events커밋).** ⑧**일정 서버 이관**(로드맵 P0-3): `events.ts`를 Firestore 구독으로 재작성, 기존 3개 자동시딩(id=ev-0705 유지→참석 무손실), id에 연도없어 내년→올해 오해하던 것 `startAt`(ms)로, 지난모임 자동접기, `nextEvent`가 없으면 null. `event-composer.tsx` 모임만들기 시트(아무나·친목). rules events 추가·배포·스모크통과(startAt누락·제목변조 DENIED). **⚠️ 웹은 아직 하드코딩(참석 공유되나 새모임 미표시) — 설치퍼널과 함께 이관. ⚠️ 22차 APK 빌드완료·실기기 미검증**(무선adb 불안정 — 113MB 재설치 안 끝난 듯, events 시딩 0). **다음 첫액션**: ①클린 재설치(`adb uninstall`→`install`) 후 홈/크루 진입해 events 시딩 3개(ev-0705/0719/0802) 확인 ②모임만들기→홈·크루 반영·참석토글 ③지난모임 접힘. **시딩 트랩 수정완료**: `events.ts` 가드를 `seeding`으로 바꿔 쓰기 성공/실패로 해제(오프라인 실패 시 재시도 가능). ⚠️ 이 수정은 22차엔 없음 → 23차부터. **그 다음: **앱 기능·완성도 계속**(회장 07-18 지시). ⚠️ **배포방향 정정**: GitHub/APK 사이드로드 접음 → **플레이스토어 정식출시**가 정답(급하지 않음·차분히 준비 트랙. $25·AAB·심사). 설치퍼널(웹 /install·APK호스팅)은 플레이스토어로 대체·보류. 웹 "준비 중" 거짓카피 정리만 소소히 남음. Firebase(데이터DB)는 용량충분·무관. 상세 `docs/ROADMAP.md` 헤더.**<br>이전 세션4: ①**GPS 상승고도**(러닝상세 타일. GPS고도 노이즈=시뮬레이터로 2번 갈아엎어 EMA+골짜기→봉우리 확정, 평지1m·언덕60→61) ②**아기자기함**: 배지 회색"미획득"→**진행바+"1.8km 남았어요"** + 획득순간 **콘페티 축하** ③**LINE Seed 폰트**(OFL·한글11172자 **TTF직접파싱 검증**. 나눔스퀘어라운드 등은 글자누락으로 탈락) + 굵기 80:40 재분류 + 모서리 토큰화 + 회장지적 "빽빽·TODAY겹침" 자간완화 ④**마스코트 남녀2종→머리띠 4종**(남/여×레드/그린팀, 확정본1개서 코드파생, 손=미튼으로 해결, 신발짤림 여백수정) ⑤**하단탭 선택색 브랜드블루** ⑥**적대검토 4렌즈** → 보안 실사용결함 수정: 멱등저장 rules영구거부(delete→재생성 폴백)·개명전파 재시도불가(순서뒤집기)·durationSec 무방비(rules배포)·앱 uid미기록(withUid). ⑦**로드맵 재정렬**(`docs/ROADMAP.md`): 아기자기함 완료선언, **다음=설치퍼널/일정이관**(웹에 APK설치경로 0=확산 불가), 채팅·다중모임 연기. `docs/AVATAR_PLAN.md`(꾸미기 큰과제). **21차까지 실기기 검증**(타이포·탭색·4종·구독반영 OK). **다음: main머지 or 설치퍼널 착수.**<br>이전(2026-07-17 세션3): (**⌚ 워치 연동 완결 + 🏁 main 머지(36커밋)** — **main HEAD=`83f2880`. 브랜치 미머지 해소.** ①**워치 살아남 — 07-09 "삼성 플랫폼 한계" 결론 폐기**: 삼성헬스 7.00.5.009가 운동·거리를 HC에 정상 기록. **폰 두고 워치만 차고 실외 러닝 → 앱에 "러닝 1개·걷기 1개·0.60km"** 반영, 오늘 뛴 거리 0.50km(러닝만)·중복 0. ②**걷기 수용**(회장 결정, `kind`로 구분·랭킹/배지선 제외) ③**유령 참석자 수정 실기기 검증**(개명 시 크루 탭 즉시 반영) ④**보안 2건**(`kind` 주입으로 남의 러닝 랭킹서 지우기 차단·배포 / 게스트 가입 시 uid 갈리던 결함 `linkWithCredential`로 수정) ⑤**`DEMO=false` → main 머지** ⑥**커뮤니티 기획서** `docs/COMMUNITY_PLAN.md`(회장 결정 3건 대기). **다음: ①경로·고도(회장 요청) ②커뮤니티 방향 결정 ③17차 APK.**<br>이전(2026-07-17 세션2): (**🔓 Auth 병목 해소 + 15차 APK 실기기 검증 + 👻 유령 참석자 버그 수정 + ⌚ 워치 결론 뒤집힘** — 브랜치 `feat/p7-2-shared-motion`. ①**Firebase Auth 활성화**(회장이 콘솔에서 익명+이메일 켬) → REST 프로브로 토큰 발급 실증 → **실기기서 게스트 로그인·재시작 후 유지까지 확인**(에이전트가 adb로 직접). ②**15차 로컬 APK 빌드·설치·검증**(07-15 수정분 전부 포함 확인: 러너네임·처방침링크·홍길동). **개명 전파 실증**(참석 1건 TESTRUN→TESTRUN2, 테스트데이터 삭제완료). ③**신규 버그 발견·수정**(커밋 `258b85d`): 개명이 다른 탭에 반영 안 돼 **유령 참석자** 발생 → `session.ts`를 구독형으로(`useMyName()`), 6개 화면 교체. **16차 APK 필요**. ④**⌚ 워치 = 우리 07-09 결론이 틀렸을 가능성 높음** — HC에 **"운동"·"총 칼로리" 항목이 생겨 있음**(07-09엔 없었음) + 삼성헬스가 **7.00.5.009 = 수정된 버전**. **회장이 워치로 걷기 1회 완료·종료함 → 검증 직전에 adb 끊겨 미확인.** ⑤ **회장 대형 비전 제시**(당근 모임식 채팅·챌린지·투표·모임운영+웹연동, 아기자기함/게임화, 케이던스) → 아래 `📋 다음 세션` 참조.<br>이전(2026-07-17 세션1): (**🚀 XSS 수정 라이브 배포 완료** — 회장 승인 후 `deploy --only hosting,firestore:rules` 실행, 라이브 스모크 8항목 통과(악성 gallery·필드주입·거리파괴 전부 DENIED / 정상 쓰기 전부 통과). **갤러리 저장형 XSS 라이브 차단 완료.** ⚠️ **남은 병목 = 회장 물리작업 2건**: ①Firebase 콘솔 → Authentication 시작하기(REST 프로브 07-17 재확인 = 여전히 `CONFIGURATION_NOT_FOUND`, **재빌드 불필요**) ②14차 APK 실기기 확인.<br>이전(2026-07-15): (**🛡️ 적대적 코드검토 4병렬 → 실사용 결함 개선 + 🔐 온보딩 처리방침·웹 익명로그인 기반** — 브랜치 `feat/p7-2-shared-motion`. 이번 세션: ①**회장 물리병목 재확인** — Firebase Auth 콘솔이 여전히 안 켜짐(REST 프로브 `CONFIGURATION_NOT_FOUND`). 켜면 **재빌드 없이** 앱·웹 로그인 동작. ②**온보딩 첫화면 처리방침 고지·링크**(openBrowserAsync, 후행TODO 해소) ③**웹 익명로그인 기반**(각 브라우저 uid 부여·`withUid`로 4개 쓰기에 곁들임, **규칙 무변경·100% 뒤로호환**, 소유기반 규칙의 발판, **배포 보류**) ④**적대적 검토 4렌즈 병렬**(보안규칙·앱크래시·GPS통계·웹퍼널) → 실사용 결함만 개선: **[치명] 갤러리 저장형 XSS**(`g.image`만 미이스케이프 → 누구나 문서 하나로 방문자 전원 코드실행 → **이중차단**: 렌더단 esc+data:image/게이트 + 규칙단 `matches('data:image/.*')`), **GPS 거리정확성 3건**(속도게이트 dt≤0 → 전구간 0km·유령거리 / 고정60m상한이 신호끊긴 뒤 정상구간 폐기 → 속도기준 대체 / 저장실패 후 트래킹 사망한 채 '계속' 노출 → `armTracking()` 추출해 되살림), **수동입력 더블탭 이중집계**(in-flight 락+disabled), **데모 이름충돌 통계오염**(개인집계서 `demo_` 제외 `mineOf`, 플레이스홀더 "김캡틴"→"홍길동"), **규칙 개방축소**(guestbook/runs update — 남의 러닝 0으로 지워 리더보드 파괴·필드주입 차단), **웹 쓰기 try/catch**. **일부러 안고침(의도적)**: delete개방·작성자명 사칭(Auth전 규칙만으론 불가) / 워치HC dedup 엣지(삼성한계로 휴면). tsc·export·웹문법 그린. 커밋 `82c3fd5`·`68bdfd9`·`4cbd9f2`·`3e1c97f`(+마감). **미머지**(브랜치). ⚠️ **XSS 수정은 라이브 반영에 배포 필요**(`firebase deploy --only hosting` + `--only firestore:rules`) — **회장 배포 승인 대기**. 앱 변경분은 다음 로컬 APK 빌드에 반영. 워치=삼성 플랫폼한계 재확인(우리 코드 정상).<br>이전(2026-07-13): (**🌊 블루 앱아이콘 + 🔐 Firebase Auth + 💥 앱 즉사 크래시 근본수정 + 🏷️ 러너 네임 개명 전파 + 🔨 로컬 APK 빌드 전환** — 브랜치 `feat/p7-2-shared-motion`. 이번 세션: ①**앱 아이콘 Azure Blue**(AI 재디자인 실패 → 기존 흰 러너 알파 역산 재합성 + 골드 스피드라인 = **D안**) ②**Firebase Auth**(게스트·이메일, RN 영속성) ③**12차 APK 실기기 즉사** → adb logcat으로 **`Component auth has not been registered yet`** 확정 → 원인=**firebase v10 × Metro package exports(dual-package hazard)** → `metro.config.js`로 해석 교정 + 초기화 페일세이프 → **13차에서 실행 검증**(에이전트가 adb로 직접 설치·기동) ④**러너 네임 개명 전파**(과거 글·참석·러닝·댓글 이름까지 batch 갱신, `identity.saveRunnerName` 단일 경로, rules `isRenameOnly()` 배포) + "내 이름"→**"러너 네임"** ⑤**웹 공개사이트 블루 재배포**(로고 SVG도 오렌지→블루) ⑥**EAS 무료 빌드 소진(8/1 리셋) → 로컬 빌드 환경 구축**(JDK17+SDK+NDK, `scripts/build-local-apk.sh`, **첫 빌드 성공** → 14차 APK). tsc 0에러 + export 그린. **⚠️ 남은 병목 1건: Firebase 콘솔 → Authentication 시작하기(이메일/비밀번호·익명) — 서버 설정, 재빌드 불필요.** 다음: 14차 실기기 → DEMO=false → main 머지. **미머지**(브랜치).<br>이전(2026-07-12): **🎨 P7-2~6 완성(5탭 IA·검색·데모·댓글·글수정) + 🌊 Azure Blue 리브랜딩 확정** — 브랜치 `feat/p7-2-shared-motion`, HEAD은 커밋 후 갱신. 이번 세션 순서: ①9차 APK 실기기 확인→`feat/p7-map-style-run-detail`→main 머지 완료 ②**P7-2** 스켈레톤·터치애니 전역 공통(`ui/skeleton`·`ui/pressable-scale`, reanimated) ③**P7-3/4/5** 2탭→**5탭 IA**(홈 Today 큐레이션·크루·러닝·랭킹·마이, Material `md=` 탭아이콘, `lib/stats`·`lib/events`) ④**P7-6** 홈 통합검색(`lib/search`) ⑤**검토용 데모데이터**(`lib/demo.ts` 6명 크루, `DEMO` 플래그 — **실배포 전 false**) + **러닝 댓글**(`comments` 컬렉션·`CommentThread`) + **방명록 글수정**(`update`, firestore.rules 배포) ⑥**메인컬러 리브랜딩**: 회장 "오렌지=당근 유사" → 파인그린 1차 적용 후 **비교 HTML 시안**(크롬) 검토 → **Azure Blue `#2563c9` 최종 확정**(accent 골드 `#c0841a`). brand.ts·웹 CSS·스플래시·DESIGN.md 전량 블루. **10·11차 APK는 그린 버전**(11차 `mEyhA2el…`). **다음 세션 최우선: ①달리는 아이콘 재디자인+배경 블루로(OpenAI/flaticon 시안) ②Firebase Auth 회원/로그인 ③블루 반영 12차 APK 빌드 → 실기기 → main 머지(DEMO=false 체크).** tsc 0에러. **미머지**(브랜치).<br>이전(2026-07-12 오전): P7 지도/상세/아이콘 → `feat/p7-map-style-run-detail`→main 머지.<br>이전: **🎨 P7 UX 폴리시 다수 완료 + 앱 아이콘 브랜드 교체 + OpenAI 이미지생성 이식** — `feat/p7-map-style-run-detail` 브랜치, HEAD=`90e16e4`. 이번 세션: ①구글맵 `customMapStyle`(파스텔·POI숨김·오렌지경로강조, 0원) ②러닝 상세 push 페이지(P7-1, NativeTabs 안 Stack 중첩 v57표준; 거리히어로·경로지도·스탯타일·삭제) ③GPS 경로 온디바이스 저장(run-path.ts, 서버 미저장 유지) ④**iOS식 슬라이드 전환**(ios_from_right·450ms·스와이프뒤로 — 회장 "아이폰 창 넘기듯" 반영) ⑤**OpenAI gpt-image-1 이식**(generate-image.mjs·`.env`·docs/OPENAI_IMAGE_GEN.md) ⑥**앱 런처 아이콘 교체**(Expo 기본 파란아이콘 → 브랜드 오렌지+흰 러너, gpt-image-1+pngjs flood-fill 합성). 8차 APK(①②③④) 실기기 확인 완료·회장 만족. **9차 APK(⑥아이콘 추가) 빌드 IN_QUEUE** — 다음 세션 첫 액션=완료 확인. tsc 0에러 + export 그린 유지. **미머지**(브랜치, 실기기 최종확인 후 main 머지 예정).<br>이전: **🏁 완성 사이클 종료 — feat/m3 → main 머지 🏁** — 성공기준 ①GPS+구글맵 라이브 ②Firebase 실시간공유 ③웹공개·처리방침 검증완료. **워치=삼성 플랫폼버그 확정**(삼성헬스가 운동·거리를 HC에 안 씀, 우리 코드 정상). main HEAD=`2964257`.)
...
- [x] **★★ `DEMO=false` → `main` 머지 완료 ✅ (2026-07-17)** — fast-forward **36커밋**, origin push + 윈도우 미러 동기화. **main HEAD=`83f2880`**. 3세션치 미머지 해소.
  - **데모는 서버에 없었음**: 데모 행은 클라이언트에서만 합성(`subscribe`가 `demo_` id로 병합) → Firestore 5개 컬렉션 전부 `demo_` 문서 **0건** 확인. 플래그만 끄면 끝(서버 청소 불필요).
  - **`app/package.json` prebuild 찌꺼기 원복**: 로컬 빌드 스크립트가 `expo prebuild`를 돌리며 scripts를 `expo run:android/ios`(bare)로 바꿔놓음 → managed 유지 원칙대로 `git checkout`. **매 로컬 빌드 후 확인할 것.**
  - ⚠️ 회장 폰의 16차 APK는 **DEMO=true 버전** — 다음 빌드(17차)부터 데모 크루원(김캡틴·이보라 등) 사라짐.
...
- **★★★ 워치 달리기 = 완결 ✅ (2026-07-17, 폰 두고 워치만 차고 실외 러닝)**
  - HC: `오후 12:59–1:01 · 삼성 헬스 · **달리기** · 2분` — **폰을 집에 두고 뛰어도** 워치가 자체 GPS로 기록 → 귀가 후 블루투스로 삼성헬스→폰→HC 자동 동기화 확인(워치 BT 모델도 GPS 내장).
  - 앱 [워치 불러오기] → **"오늘 러닝 1개 · 걷기 1개 · 0.60km 불러왔어요"**
  - **집계 분리 실증**: 오늘 뛴 거리 `0.00 → 0.50km`(0.60 중 **러닝만**) / 누적 `62.7 → 63.2km`(+0.5) / 기록 `11 → 12`(+1, 걷기 안 셈).
  - **Firestore 최종 확인**: 회장 워치 기록 **정확히 2건·중복 0** — `kind=walk 0.098km/106초` + `kind 없음(=run) 0.501km/155초`. **'run'은 필드 생략** 설계(뒤로호환)까지 실물로 확인. 이미 있던 걷기가 재동기화로 **중복 생성되지 않음** = 멱등 저장 실기기 검증.
  - **개명 전파 실전 확인**: 회장이 러너 네임을 "쫀쫀샷"으로 저장 → 에이전트 테스트값(TESTRUN2X)으로 들어와 있던 워치 걷기 기록이 **자동으로 회장 이름으로 이전**. 전 컬렉션 TESTRUN 잔재 **0건**(청소 불필요했음).
  - ⚠️ **알려진 갭(미수정)**: 폰을 들고 나가 **앱 [러닝 시작](GPS)과 워치를 동시에** 쓰면 같은 러닝이 `source:'gps'` 1건 + `source:'healthconnect'` 1건 = **2건으로 이중 계상**된다. 현재 dedup(`healthconnect.ts` byOrigin)은 **HC 내부의 멀티앱 중복만** 막고 앱GPS↔워치는 못 거른다. → 후행 과제(시간대 겹침 + 거리 근사로 병합 판정, 또는 사용자에게 "이 러닝은 워치 것과 같나요?" 확인).

### 📋 ★★ 다음 할 일 (2026-07-17 세션3 마감 · main 머지 완료 후)

> **adb**: `adb connect 172.30.1.60:<포트>` — **무선디버깅 보안 해제가 30분마다 풀려 포트가 매번 바뀐다**(회장 확인). refused면 포트스캔: `for p in $(seq 30000 49999); do timeout 0.3 nc -z 172.30.1.60 $p && echo $p; done`(2000개씩 병렬). 화면 꺼지면 스크린샷이 검정 → `adb shell input keyevent 224`로 깨울 것. **한글은 `input text`로 못 침(ASCII 전용)**.

1. **★ 경로·고도 (회장 요청 — "어디부터 어디까지 뛰었는지, 오르막·내리막도 표시되나")**
   - **현황(조사 완료)**: 앱 GPS 러닝은 경로 지도 **있음**(`run-path.ts`, **온디바이스만** — P5 "GPS 좌표 서버 미저장" 원칙). **워치 러닝은 경로 없음** — 우리가 `ExerciseSession`·`Distance`·`HeartRate` **3개만** 읽어서. **고도는 어디에도 없음**(`Run` 스키마에 필드 자체가 없고, 앱 GPS도 `loc.coords.altitude`를 **받고서 버림** — `{lat,lng}`만 챙김).
   - **(a) 앱 GPS 고도 = 지금 바로 가능·우리 손 안**: `live-run.tsx`가 이미 받는 위치에서 altitude만 안 버리면 됨 → 누적 상승고도 + 오르막·내리막 프로필을 러닝 상세에. `run.ts`에 필드 추가, `run-path.ts` 저장 구조에 alt 추가.
   - **(b) 워치 경로·고도 = 조사 먼저**: 라이브러리는 지원함 — `ExerciseSessionRecord.exerciseRoute?: ExerciseRoute`, `ExerciseRoute { type?: DATA|NO_DATA|**CONSENT_REQUIRED**, route: Location[] }`, `Location { time, latitude, longitude, **altitude?** }` → **경로 읽으면 지도+고도 둘 다 해결**. **단 ①삼성헬스가 경로를 실제 HC에 쓰는지 미확인**(HC 데이터 브라우저가 경로를 렌더 안 해서 눈으로 확인 불가 → **코드로 프로브해야 함**) **②별도 권한 필요**(`CONSENT_REQUIRED` = READ_EXERCISE_ROUTE 계열). 워치 때처럼 삼성이 안 쓸 수도 있음.
   - **HC 카테고리 실측(07-17)**: 거리·걸음수·속도·운동·총칼로리 + 심박수. **고도(ElevationGained) 항목 없음** = 삼성헬스가 고도를 별도 레코드로 **안 씀**(라이브러리는 `ElevationGainedRecord` 지원하나 소스가 없음).
   - ⚠️ **개인정보**: 경로=좌표. P5 게이트에서 "서버 미저장"으로 확정한 사안 → 워치 경로를 읽더라도 **온디바이스 유지**가 원칙. 바꾸려면 처리방침 개정 + 자문 재검토.
2. **★ 커뮤니티 방향 결정** — `docs/COMMUNITY_PLAN.md` §5 **회장 결정 3건**: (A)모임 **단일 vs 다중**(추천: 단일, 스키마에 `crewId` 자리만) (B)게스트 채팅 허용 여부 (C)착수 지점 — 아기자기함 / 일정 만들기 / 채팅. **채팅·투표는 uid 소유 기반이 전제**(사칭·무단삭제·중복투표). `linkWithCredential`은 07-17에 선반영 완료.
3. **17차 APK** — DEMO=false 반영본(데모 크루원 사라짐) + 경로·고도 작업분.
4. ⚠️ **알려진 갭**: 앱 GPS + 워치 동시 사용 시 같은 러닝이 **2건 이중 계상**(dedup은 HC 내부 멀티앱만 커버).

### 📋 (완료) 다음 세션 최우선 — 2026-07-17 세션2 마감 시점
