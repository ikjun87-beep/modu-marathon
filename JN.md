...
> 최종 업데이트: 2026-07-06 (대량 진전 — ①Firebase 실연결 완결(웹·앱·실기기) ②웹 Firebase Hosting 공개배포 `modu-marathon.web.app` ③처리방침 실값+웹게시 ④앱 UI 3버그 수정 ⑤워치 크래시 근본원인 확정(adb logcat)+수정 4차 빌드. **남은 건 회장 실기기 워치 재확인 + 비 그치면 GPS → main 머지**.)
...
- [ ] **워치 실기기 재확인**: 4차 APK(`36qaOAmt…`)로 [워치 불러오기] → 크래시 없이 Health Connect 권한창 뜨고 아침 트레드밀 러닝 로드되는지. (adb 무선 여전히 연결가능: `$CLAUDE_JOB_DIR/tmp/platform-tools/adb` — 만료 시 재다운로드. 크래시 시 `adb logcat -b crash -d`로 즉시 로그.)
- [ ] **GPS 실기기 검증**: 비 그치면 실외 1km 코스에서 [러닝 시작] → 거리 ±5%·드리프트·페이스 확인 (`docs/QA_M3_DEVICE.md ②`).
- [ ] **위 2개 통과 → `feat/m3-live-run-watch` → `main` 머지** → 회장 추인 = 완성 사이클 종료.
