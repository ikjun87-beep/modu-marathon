...
- [ ] **★★★ 최우선 — 적대적 검토 XSS 수정 배포**(2026-07-15): 갤러리 저장형 XSS는 **지금 라이브 사이트에 열려 있음**. 코드는 고쳤으나(커밋 `3e1c97f`) 라이브 반영엔 배포 필요 — **회장 "배포해" 승인 시**: `npx firebase-tools deploy --only hosting` + `npx firebase-tools deploy --only firestore:rules`(로그인 캐시됨) → 후 라이브 스모크(정상 렌더 + 악성 gallery create가 규칙에 거부되는지). **rules 변경분(guestbook/runs update 축소·gallery image 검증)도 이때 함께 배포됨** → 정상 쓰기(참석·방명록·러닝저장·러너네임 전파)가 안 막히는지 확인.
- [ ] **★★ 다음 세션 최우선**
  1. **⚠️ 회장 물리작업(유일한 병목)**: Firebase 콘솔 → **Authentication → 시작하기 → Sign-in method → 이메일/비밀번호 + 익명 사용설정**. 안 켜면 로그인·게스트 둘 다 `CONFIGURATION_NOT_FOUND`로 막힘(REST 프로브로 재확인 07-15). **앱 재빌드 불필요 — 서버 설정**. 켠 뒤 REST 프로브(`curl -X POST "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=<웹apiKey>" -d '{"returnSecureToken":true}'`)로 확인. 켜지면 웹 익명로그인도 `_uid` 채워짐(withUid 발판 활성).
  2. **14차 APK 실기기 확인**(회장, `C:\Users\JUN\modu-marathon-14.apk` — **기존 앱 삭제 후 설치**): ①앱 정상 실행 ②5탭 블루 ③**러너 네임 바꾸면 과거 글·참석 이름도 바뀌는지** ④마이 탭 → 계정 → 로그인/게스트 ⑤**앱 껐다 켜도 로그인 유지**(RN 영속성 핵심). ⚠️ **주의: 07-15 적대적 검토 앱 수정(GPS·수동락·데모오염·온보딩 처리방침)은 14차 APK에 아직 없음** — 다음 로컬 APK 빌드(15차)에 반영됨.
  3. 만족 시 → **`demo.ts` DEMO=false** → `feat/p7-2-shared-motion` → `main` 머지.
