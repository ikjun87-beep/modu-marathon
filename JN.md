...
- [ ] **★★ 다음 세션 최우선**
  1. **⚠️ 회장 물리작업(유일한 병목)**: Firebase 콘솔 → **Authentication → 시작하기 → Sign-in method → 이메일/비밀번호 + 익명 사용설정**. 안 켜면 로그인·게스트 둘 다 `CONFIGURATION_NOT_FOUND`로 막힘(REST 프로브로 확정). **앱 재빌드 불필요 — 서버 설정**. 켠 뒤 REST 프로브(`curl -X POST "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=<웹apiKey>" -d '{"returnSecureToken":true}'`)로 확인 가능.
  2. **14차 APK 실기기 확인**(회장, `C:\Users\JUN\modu-marathon-14.apk` — **기존 앱 삭제 후 설치**): ①앱 정상 실행 ②5탭 블루 ③**러너 네임 바꾸면 과거 글·참석 이름도 바뀌는지** ④마이 탭 → 계정 → 로그인/게스트 ⑤**앱 껐다 켜도 로그인 유지**(RN 영속성 핵심).
