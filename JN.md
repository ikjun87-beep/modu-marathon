...
- **9차 APK 빌드 트리거됨**(ID `ae199551-eb2e-4438-b0f1-237284c3dab5`, preview) — **아이콘 + 전환애니(ios_from_right) + 지도스타일 + 러닝상세 전부 포함**. 8차를 대체. 세션 마감 시점 **IN_QUEUE**(EAS 큐 혼잡) → **다음 세션 첫 액션 = `npx eas-cli build:view ae199551… --json`으로 완료 확인 후 APK 링크 확보**. 완료되면 회장 재설치(기존앱 삭제) → ①홈런처 새 오렌지 러너 아이콘 ②스플래시 ③상세 슬라이드 전환 ④지도 파스텔 확인.
