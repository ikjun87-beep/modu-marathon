...
> 최종 업데이트: 2026-07-07 (①워치 크래시 완전해결(4차) ②실시간 GPS **구글맵 경로표시**(5차) ③**워치 미등록 근본원인 확정+수정 완료** — Android 14+ 필수 `VIEW_PERMISSION_USAGE` activity-alias 누락이 원인, 커스텀 플러그인으로 주입·매니페스트 검증 완료(adb 불필요). **남은 건 6차 APK 빌드(지도+워치수정) → 실기기 확인 → main 머지**.)
...
- [ ] **★ 6차 빌드 트리거**(회장 승인 필요): `cd app && npx eas-cli build -p android --profile preview` → 지도(정상)+워치수정 묶인 APK. 완료 시 링크 확보.
- [ ] **6차 APK 실기기 확인**: [워치 불러오기] → HC 권한창 표시 + HC 앱목록에 "모두의 마라톤" 등장 + 당일 러닝 로드. 여전히 안 뜨면 그때 adb logcat(런처 준비됨).
- [ ] **GPS+지도 통과(+워치) → `feat/m3-live-run-watch` → `main` 머지** → 회장 추인 = 완성 사이클 종료.
