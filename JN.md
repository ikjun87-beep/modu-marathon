...
- [ ] **★ 워치 adb 확정 진단**: HC 목록에 "모두의 마라톤" 없음 = 앱 미등록 확정. 회장 무선디버깅 페어링 → `<scratchpad>/platform-tools/adb`로 `adb pair`→`connect`→`logcat -c`→[워치 불러오기]→`logcat -d`. requestPermission이 권한 UI를 띄우는지/왜 안 뜨는지 로그로 확정 → 수정.
- [ ] **워치 수정 → 6차 빌드**(지도 정상 + 워치 수정 묶기) → 실기기 확인.
