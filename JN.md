...
> 최종 업데이트: 2026-07-07 (①워치 크래시 완전해결 확인(4차 APK) — 이제 권한부여 이슈만 ②실시간 GPS 러닝 **구글맵 경로표시** 신규기능 ③워치 requestPermission 후 부여확인 로직 추가 → **5차 빌드 진행중**(지도+워치권한안내). **남은 건 5차 APK로 워치권한·GPS+지도 실기기 확인 → main 머지**.)
...
- **5차 빌드**: `c38ea92e-5549-48d3-9e35-14a822aaec06` (preview APK). 완료 시 APK 링크 → 지도+워치권한 실기기 확인. tsc 0에러 + expo export web 그린.
...
- [~] **5차 APK 빌드**(`c38ea92e…`) 완료 대기 → 설치. (지도 + 워치 권한부여 안내 반영)
- [ ] **워치 권한 재확인**(5차 APK): [워치 불러오기] → 권한창 뜨고 부여되는지. 여전히 SecurityException/미부여면 → Health Connect 앱 목록에 "모두의 마라톤" 있는지 확인 + **무선 adb logcat**(개발자옵션→무선디버깅→페어링코드→`adb logcat -d`)으로 requestPermission이 권한창을 띄우는지 확정.
- [ ] **GPS+지도 검증**(5차 APK): [러닝 시작] → 지도에 경로 실시간 표시, 거리 ±5%·드리프트·페이스 (`docs/QA_M3_DEVICE.md ②`). 구글맵 회색이면 Maps SDK 사용설정/결제/키 제한 확인.
- [ ] **위 통과 → `feat/m3-live-run-watch` → `main` 머지** → 회장 추인 = 완성 사이클 종료.
