...
- [ ] **★★ 다음 세션 최우선 3건**:
  1. **달리는 아이콘 재디자인 + 배경 블루로** — 런처아이콘·스플래시 러너 배경이 아직 오렌지. OpenAI(`generate-image.mjs` 연결됨) 생성 or **flaticon.com 벡터(SVG)** 다운로드 → 브랜드 블루 적용. 두 방식 시안 비교 후 채택. app/assets/images/ 교체.
  2. **Firebase Auth 회원/로그인** — 회장 결정: 회원정보 저장=**Firebase Auth**(Supabase 신규연결 대신, 이미 연결된 단일 백엔드). 게스트/이메일 로그인 + RN 영속성(`getReactNativePersistence`). 이름기반 신원을 실계정으로 승격. (Supabase는 크로스프로젝트 공유 필요시에만.)
  3. **블루 반영 12차 APK 빌드** → 실기기 확인 → 만족 시 `feat/p7-2-shared-motion`→main 머지. **머지 전 체크: `demo.ts` DEMO=false**(검토용 데모 끄기).
