...
export function Avatar({ name, size = 38, me = false, ring = true, tier }: Props) {
  ...
  // 성장 링 — 등급이 오르면 **굵기**가 오르고, 마지막에만 색이 골드로 바뀐다.
  //   1등급(0개)  옅은 회색 · 얇게   — 위축시키지 않게
  //   2등급(1~2개) 내 팀색 · 얇게
  //   3등급(3~4개) 내 팀색 · **굵게**
  //   4등급(5개)   골드 · 굵게        — 전부 획득
  //
  // ⚠️ 3등급에 `Brand.brand`(액션색)를 쓰지 말 것 (2026-07-30 R19 독립 채점 지적).
  //   이 파일 위쪽에서 "링에 브랜드색·골드를 쓰지 말 것"을 못 박아 놨는데 여기가 그걸 어겼다.
  //   더 나쁜 건 위치다 — my.tsx의 `heroCam`(사진 변경 카메라, **진짜 액션 버튼**)이
  //   `Brand.brand` 배경으로 이 링에 **겹쳐서** 그려진다. 3~4개를 딴 사용자에게는
  //   성장 링(정보)과 카메라 버튼(액션)이 같은 초록으로 맞닿아 한 덩어리로 보였다.
  //   → 색을 새로 늘리지 않고(링 3색 규칙 유지) **굵기로** 3등급을 나른다.
  //   4등급의 골드는 "전부 획득" 성과 신호라 규칙에 맞다(골드=순위·챌린지·성과 전용).
  if (tier != null) {
    if (tier >= 4) { color = Brand.gold; border = Math.round(size * 0.075); }
    else if (tier === 3) { border = Math.round(size * 0.075); } // 색은 팀색 유지, 굵기만 올린다
  ...
  //
  // ⚠️ **이 36px 하한은 마스코트에만 적용한다**(2026-08-01 실기기 검증에서 잡음).
  //   위 근거 두 가지가 사진에는 하나도 해당하지 않는다:
  //    ① "외곽선이 뭉개진다"는 **선화(線畵)** 이야기다. 사진엔 뭉개질 얇은 외곽선이 없고,
  //       256px로 저장된 얼굴을 30px로 줄이는 건 메신저가 매일 하는 평범한 축소다.
  //    ② "나만 예외라 일관성을 잃는다"도 **2026-07-31 크루 공유 전환으로 소멸했다.**
  //       이제 누구나 사진을 가질 수 있으니 사진을 그리는 건 자기예외가 아니라 균일한 규칙이다.
  //
  //   하한을 사진에까지 걸어 둔 대가가 컸다. 이 앱에서 Avatar가 36px 이상인 곳은
  //   **마이 탭 내 히어로(72)와 랭킹 시상대 1~3위(52/42)뿐**이다. 정작 크루원이 서로를
  //   매일 보는 자리 — 홈 크루 소식(30) · 댓글(30) · 랭킹 행(34) · 모임 참석자(22) — 는
  //   전부 하한 아래라, **사진을 등록해도 크루원 눈엔 여전히 이니셜만 보였다.**
  //   "크루가 서로 얼굴을 보게" 하려고 서버 공유로 전환한 L3 결정(2026-07-31)이
  //   시상대 3등 안에 든 사람에게만 이행되고 있었던 셈이다.
