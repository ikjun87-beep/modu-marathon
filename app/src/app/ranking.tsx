                    ...
                    return (
                      <View key={r.name} style={styles.podCol}>
                        ...
                        {/* ⚠️ 시상대는 목록 행과 **다른 규칙**이어야 한다.
                            ...
                            → **한 줄 고정 + 말줄임**. 여기까지 세 번 돌아온 결론이다:
                              ① 2줄 허용 → 1위 칸만 길어져 2·3위와 정렬이 무너짐
                              ② adjustsFontSizeToFit(최소 75%) → 19자가 다 보이지만 안 읽힘
                              ③ 최소 85%로 상향 → **안드로이드는 minimumFontScale을 제대로
                                 지키지 않아** 크기가 그대로였다(iOS와 구현이 다르다).
                            81dp 칸에 19자를 담는 방법은 없다. 시상대의 목적은 "누가 1등인지
                            한눈에"이므로 **잘려도 크게 읽히는 쪽**을 택한다 — 2·3위와 같은
                            크기라 위계도 지켜진다. **풀네임은 4위 이하 목록(3줄)이 책임진다.** */}
