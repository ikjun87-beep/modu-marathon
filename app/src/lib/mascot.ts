 ...
 *
 * 4종 = 리본 유무 × 팀 색(레드/그린 조끼). **네 장 모두 한 장의 원본에서 코드로 파생**한다:
 * 팀 색은 `scripts/recolor-mascot-vest.mjs`(조끼 hue만 밝기비 유지한 채 재도색),
 * 리본은 `scripts/add-mascot-ribbon.mjs`(머리 오른쪽 위에 조끼색 리본 합성).
 * AI로 두 번 그리면 실루엣이 미묘하게 달라져 "같은 양의 다른 모습"이 아니라 다른 양이 된다.
 *
 * 2026-07-31 구분을 **울 모양(동글/땋은) → 리본**으로 교체(회장 지시). 땋은 머리가 양의 곱슬
 * 울과 겹쳐 어색했다 — 양은 머리카락이 아니라 울로 덮인 동물이라, 헤어스타일로 가르려는 것
 * 자체가 형태와 싸우는 일이었다. 리본은 울 위에 **얹히는 소품**이라 충돌이 없고,
 * 머리 윤곽 밖으로 삐져나와 **36px 아바타에서도 실루엣이 갈린다**.
