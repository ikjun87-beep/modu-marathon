...
- 🧭 **격리보다 소유(uid)가 먼저다 — 2026-08-14 세션13 방향 전환.** 세션12가 "다음 첫액션 = TIERS 3층 컷오버"로 남겼으나 **철회했다.** `docs/COMMUNITY_PLAN.md §2-3`에 훨씬 싼 경로가 이미 있었다 — **uid 소유 + 유예 조항**(`!('uid' in resource.data) || resource.data.uid == request.auth.uid`), 자체 견적 **반나절~하루**. **소유와 격리는 분리 가능하고 급한 건 소유 쪽이다.**
  - **단일 크루로 시작 · `crewId` 자리만 예약**(오너 확정). 크루 객체·초대·권한·격리·탐색은 **PHASE 4**. `COMMUNITY_PLAN §5 결정A`가 원래 권고한 자리로 되돌아왔다.
  - **「비공개 테스트를 격리 완료 전에 금지」는 완화됐다.** 그 근거는 안전이 아니라 **마이그레이션 비용**이었고, 단일 크루면 마이그레이션 자체가 없다(다중으로 가도 12명×14일은 수백 건이라 여전히 쌈). **PHASE 1(S1~S5) 완료가 실질 조건**이다 — **S1~S4 완료(2026-08-15), S5(백그라운드 GPS)는 구조적 장애물로 미완료**(아래 참조).
  - ⛔ 여전한 사실: 8개 컬렉션이 전부 `read: if true`이고 문서에 소속 표시가 없어 **앱을 깐 전원이 하나의 크루**다. 두 번째 크루를 받으려면 격리가 선행한다.
...
- **신원**: 문서 작성자는 `name` 문자열로 박제된다(웹·앱 공유 스키마). 러너 네임 변경은 반드시 `lib/identity.ts`의 `saveRunnerName()` 단일 경로로 — 과거 글·참석·러닝·댓글의 이름까지 함께 갱신해야 기록이 유실되지 않는다.
  ...
  - ✅ **소유권 delete — S4에서 적용 완료(2026-08-15).** `firestore.rules`에 `isOwnerOrLegacy()` 함수 신설 — `!('uid' in resource.data) || resource.data.uid == request.auth.uid`. **`runs`·`guestbook`·`gallery` 3곳**(삭제 UI가 있는 컬렉션)의 `delete`를 이걸로 교체했다. `uid` 없는 레거시 문서는 여전히 누구나 지울 수 있다(자연 소멸 대상이라 그대로 둠). UI 쪽은 `crew.ts`의 `isMine(row, myName)` 헬퍼(uid 있으면 uid로, 레거시면 이름으로 판단 — rules와 같은 축)로 `crew.tsx`·`gallery-section.tsx`·`explore/run/[id].tsx` 3곳에 가드를 걸어, 남의 문서엔 삭제 버튼 자체가 안 뜬다. **`attendance`·`claps`·`comments`·`events`·`profiles`의 delete는 여전히 `if true`**(S4 범위 밖 — 참석·박수 취소는 소유권보다 다른 방식이 맞을 수 있어 다음 라운드로 유예).
...
- 🚧 **S5(백그라운드 GPS 러닝) — 구조적 장애물로 미완료(2026-08-15).** `expo-task-manager` + `Location.startLocationUpdatesAsync`로 구현했고(`lib/live-tracking.ts` 신설, `ACCESS_BACKGROUND_LOCATION` 없이 Foreground Service만으로 화면 OFF 추적 시도), **위치 업데이트가 처음 들어오는 순간 앱이 크래시한다.** 원인 확정(실기기 crash 로그): `node_modules/expo-task-manager`의 `TaskManagerUtils.java:205`가 `JobInfo.Builder.setPersisted(true)`를 **하드코딩**(옵션 없음) — 이게 `RECEIVE_BOOT_COMPLETED` 권한을 요구하는데, 우리 `app.json`의 `android.blockedPermissions`가 그 권한을 명시적으로 차단해뒀다. 충돌 결과: `IllegalArgumentException: Requested job cannot be persisted without holding android.permission.RECEIVE_BOOT_COMPLETED permission`.
  - **해결 전 오너 판단 필요** — ①`blockedPermissions`에서 `RECEIVE_BOOT_COMPLETED` 제거(가장 간단, 플레이스토어 심사 시 용도 설명 필요) ②`patch-package`로 라이브러리 소스 패치(`setPersisted(false)`, 신규 의존성+SDK 업그레이드마다 깨질 위험) ③백그라운드 추적 포기, foreground만 유지(S5 목표 자체를 접음). **임의로 고르지 않고 보류 중.**
  - 코드 자체(`live-tracking.ts`의 거리·경로·상승고도 계산, `live-run.tsx`의 armTracking 재구성)는 유효 — 위 3안 중 하나가 정해지면 그 위에서 이어가면 된다.
