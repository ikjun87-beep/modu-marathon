...
- 🧭 **격리보다 소유(uid)가 먼저다 — 2026-08-14 세션13 방향 전환.** 세션12가 "다음 첫액션 = TIERS 3층 컷오버"로 남겼으나 **철회했다.** `docs/COMMUNITY_PLAN.md §2-3`에 훨씬 싼 경로가 이미 있었다 — **uid 소유 + 유예 조항**(`!('uid' in resource.data) || resource.data.uid == request.auth.uid`), 자체 견적 **반나절~하루**. **소유와 격리는 분리 가능하고 급한 건 소유 쪽이다.**
  ...
  - **「비공개 테스트를 격리 완료 전에 금지」는 완화됐다.** 그 근거는 안전이 아니라 **마이그레이션 비용**이었고, 단일 크루면 마이그레이션 자체가 없다(다중으로 가도 12명×14일은 수백 건이라 여전히 쌈). **PHASE 1(S1~S5) 완료가 실질 조건**이다.
...
- 🔴 **자동 감지 산책이 크루 피드에 자동 게시된다 — 미수정 결함(S2 대상).** 삼성헬스가 **사용자가 버튼을 안 눌러도** 걷기를 자동 감지해 Health Connect에 쓰고(2026-08-14 실기기 확인) → `watch-autosync.ts`가 앱 열 때 30분 간격으로 **조용히** 가져오고 → `healthconnect.ts:29`가 WALKING(79)→`kind:'walk'`로 저장하고 → **`feed.ts:62~73`의 runs 루프가 걷기를 안 걸러** 홈 타임라인에 게시하고 → `read: if true`라 전원이 본다. **가설이 아니라 이미 발생한 사실**(`runs`의 07-24 `walk` 2건 = 시속 3.6~3.8km 산책). 동의 문구(`explore/index.tsx:152~`)에 **자동 감지 산책이 포함된다는 고지가 없다.**
  - **처방은 두 개를 다 해야 한다** — ⓐ`silent` 경로에서 walk 수집 차단(= 고지 없는 자동 수집) ⓑ`feed.ts`에서 walk 노출 차단(= 출석부 노이즈). **서로 다른 문제라 하나만 하면 나머지가 남는다.**
- **워치 집계 = "쓰던 앱 그대로"가 카피의 정본.** 앱 이름을 나열하지 말 것 → **「쓰던 러닝앱 그대로 쓰세요. 한 번만 연결하면 기록은 여기로 모입니다.」**
  ...
  - ⚠️ HC 유입 기록이 화면에 **전부 "갤럭시워치"로 표시**된다(`explore/index.tsx:38`·`[id].tsx:30`·`search.ts:23`). 스트라바·나이키런 기록에도 같은 라벨이 붙어 **거짓말이 된다** → S3에서 `sourceApp`(`dataOrigin.packageName`) 저장·표시로 정정. 지금 `healthconnect.ts:162`가 그 값을 **읽고도 버린다.**
