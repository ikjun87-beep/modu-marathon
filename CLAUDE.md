...
- ✅ **크루 데이터 격리 — PHASE 2 운영 컷오버 완료(2026-08-17 세션19~22).** 세션13의 "격리보다 소유 먼저" 방향(uid 소유 + 유예 조항 선행)을 거쳐, 세션19 조사로 **B안(서브컬렉션) 채택**을 확정하고 세션21~22에서 실제 배포·마이그레이션·실기기 서버 확정까지 끝냈다. 8개 컬렉션이 더 이상 전부 `read: if true`가 아니다 — 아래 최종 구조 참고.
  ...
  - **✅ PHASE 2-C 크루 운영 MVP 4/4 완료 + 실기기 검증(2026-08-18 세션23).** 크루 공지(`app/src/lib/crew-notice.ts` + `crew/index.tsx` 헤더 카드, 크루장·관리자만 작성) · 역할/관리자(`app/src/lib/crew-roster.ts` + `app/src/app/crew/roles.tsx`, 크루장이 멤버를 관리자로 승격/강등) · 출석 이력 조회(`app/src/app/crew/attendance.tsx`) · 초대코드 발급·가입(`app/src/lib/invite.ts` + `app/src/app/crew/invite.tsx`, 발급은 크루원 누구나). `crew.tsx`는 `crew/{_layout,index,attendance,invite,roles}.tsx`로 라우팅 구조 전환(`explore/`와 같은 push-스택 패턴). rules 112/112·tsc 0. **`firestore.rules` 프로덕션 배포 완료** — 실기기(크루장 계정)로 공지 작성·역할 승격/강등까지 서버 왕복으로 확인.
    - **크루장 판정은 오직 `crews/{id}.ownerUid` 하나다.** `members/{uid}.role`엔 `'admin'|'member'`만 존재하고 `'owner'` 자기선언은 rules가 완전히 막는다(세션20~21 마이그레이션 스크립트가 자기 자신에게 `role:"owner"`를 부여했던 사고의 재발 방지) — 이 필드가 두 번째 진실의 소스가 되지 않게 하는 설계다.
    - **`crews/modu.ownerUid`는 세션23에 오너가 Firebase 콘솔에서 직접 수정했다**(`R8oPHL5mQdb4D4URuWdqBrHRy3b2` — 실기기 uid). 이 필드는 앱의 어떤 경로로도 못 바꾸는 불변 필드라(공지 수정만 rules가 허용) 소유권 재지정은 항상 콘솔 수동 편집으로만 한다.
    - **역할 화면의 이름 표시**: `members` 문서는 원래 이름이 없어(uid만) `crew-roster.ts`가 기존 runs·comments·claps·profiles·guestbook·gallery·attendance를 uid로 역조회해 최선을 다해 채운다 — 활동이 전혀 없는 신규 멤버는 "이름 미확인"으로 정직하게 남을 수 있다. `ensureCrewMembership()`·`saveRunnerName()`(개명 전파, `crew.ts`의 `syncMyMembershipName`)이 이제 멤버십 문서에도 이름을 함께 쓴다.
    - ⚠️ 초대코드 생성은 아직 `expo-crypto`(CSPRNG) 대신 `Math.random()` 잠정 사용 — 코드가 실제 접근 통제가 아니라 공유 편의인 동안만 감수, 다중 크루(PHASE 4)로 코드가 진짜 게이트가 되면 교체 필요.
